...
def build_model_with_time(horizon_hours):
    ...
    #     pyo.Constraint(
    #         model.time,
    #         rule=lambda model, t: model.electrolyzer_power_input[t] <= model.power_available[t],
    #     ),
    # )

    lefts = [model.electrolyzer_power_input]
    rights = [model.power_available]
    sum_lefts = lambda t: sum(left[t] for left in lefts)
    sum_rights = lambda t: sum(right[t] for right in rights)
